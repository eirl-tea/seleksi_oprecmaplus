...
const peserta = [
      {
        name: "Fajar Assiddiqi",
        email: "Fajarsubuh@gmail.com",
        phone: "081234567890",
        class: "10 Z",
        passed: false,
        club: "MULTIMEDIA"
      ...
      {
        name: "Farah Alexandria",
        email: "bulekimup@gmail.com",
        phone: "0873912381109",
        class: "10 M",
        passed: true,
        club: "ROBOTIK"
    ...
    function checkResult() {
      const name = document.getElementById("nama").value.trim().toLowerCase();
      const email = document.getElementById("email").value.trim().toLowerCase();
        
      const result = peserta.find(p =>
        p.name.toLowerCase() === name && p.email.toLowerCase() === email
      );

      if (result) {
        if (result.passed) {
          localStorage.setItem("dataPeserta", JSON.stringify(result))
          window.location.href = "biru.html";
        } else {
          localStorage.setItem("dataPeserta", JSON.stringify(result))
          window.location.href = "merah.html";
        }
      } else {
